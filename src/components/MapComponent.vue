...
<script lang="ts">
  ...
  export default class MapComponent extends Vue {
    ...
    public $refs!: {
      map: HTMLElement & SVGGraphicsElement;
    };

    mounted() {
      // this.map = document.getElementById('map')! as HTMLElement & SVGGraphicsElement;
      this.$refs.map.addEventListener('click', this.clickHandler);
    }
    ...
    clickHandler(evt: MouseEvent) {
      ...
      if (target.nodeName !== 'path') {
        this.$refs.map.setAttribute("viewBox", this.viewBoxDefault);
      } else {
        ...
        const viewBox = this.$refs.map.getAttribute('viewBox')!.split(' ').map(el => parseFloat(el));
        ...
        const matrix = this.parent.getScreenCTM()!.inverse().multiply(this.$refs.map.getScreenCTM()!);
        ...
        const scaledOffsetX = absoluteOffsetX + viewBox[2] * (1 - scale) / 2 - (bbox.width / 5);
        ...
        this.$refs.map.setAttribute("viewBox", `${scaledOffsetX} ${scaledOffsetY} ${scaledWidth} ${scaledHeight}`);
      }
    }
  };
</script>
